...

{
  ...
  async function sleepOneByOneAsyncAwaited() {
    // Писать код здесь
    const res_1 = await sleep500();
    const res_2 = await sleep200();
    const res_3 = await sleep100();
...

{
  ...
  function sleepOneByOnePromised() {
    // Писать код здесь
    return sleep500().then((res1) =>
      sleep200().then((res2) => sleep100().then((res3) => res1 + res2 + res3))
    );
  }
...

{
  ...
  function sleepOneByOne() {
    ...
    orderedSleeps.forEach((p) => {
      resPromise = resPromise.then((res) => p().then((res1) => res + res1));
    });
...

{
  ...
  async function sleepParallel() {
    // Писать код здесь
    return await Promise.all([sleep100(), sleep200(), sleep500()]).then((res) =>
      res.join("")
    );
  }
...

{
  ...
  function execWithTimeout() {
    ...
    return Promise.race([promiseLike, promiseTimeout]).catch((err) => {
      promiseLike.cancel();
      throw err;
    });
  }
